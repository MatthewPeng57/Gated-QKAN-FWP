...
class Worker(mp.Process):
	...
	def run(self):
		...
		while self.g_ep.value < MAX_EP:
			...
			fwp_param = self.lnet.fwp.fwp_cell.initial_fast_params(batch_size = 1)
			buffer_s, buffer_a, buffer_r = [], [], []
			ep_r = 0.
			while True:
				a, fwp_param = self.lnet.choose_action(v_wrap(s[None, :]).double(), fwp_param)
				...
				if total_step % UPDATE_GLOBAL_ITER == 0 or done:  # update global and assign to local net
					...
					push_and_pull(self.opt, self.lnet, self.gnet, done, s_, (fwp_param,), buffer_s, buffer_a, buffer_r, GAMMA)
					buffer_s, buffer_a, buffer_r = [], [], []
