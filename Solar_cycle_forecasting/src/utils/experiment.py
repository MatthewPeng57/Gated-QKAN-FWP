...
"""Experiment bookkeeping: run-directory layout and provenance snapshots."""
...
def build_result_path(args, experiment_root: Path):
	...
	# Guarantee the resolved path stayed inside the experiment root.
...
def save_environment_snapshot(result_path: Path):
	"""Write a provenance snapshot of the running environment.

	Produces:
	- python_info.txt       (interpreter version + executable)
	- requirements.txt      (pip freeze)
	- environment.yaml      (conda env export, when conda is available)
	- conda_list.txt        (conda list)
	- conda_explicit.txt    (conda list --explicit; same-platform restore)
	- conda_env_name.txt    (active conda environment name)
	"""
	result_path.mkdir(parents=True, exist_ok=True)

	# 1) Interpreter info (always available).
	...
	# 2) pip freeze — always worth saving, since a conda export can miss
	#    pip-only packages.
	try:
	...
	# 3) conda artifacts, only when conda is present. Skipped automatically
	#    outside a conda environment.
	try:
		# conda is not always on PATH, but usually is inside a conda env.
		subprocess.run(["conda", "--version"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)

		# conda env export (includes a pip: block; requirements.txt is still
		# saved separately).
		...
		# conda list — quick to diff between runs.
		with open(result_path / "conda_list.txt", "w") as f:
			subprocess.run(["conda", "list"], stdout=f, stderr=subprocess.DEVNULL, check=True)

		# 4) Explicit spec — restores the exact same environment on the same
		#    platform, but is platform-locked and may not resolve elsewhere.
		...
		# Active conda environment name, recorded for convenience.
		env_name = os.environ.get("CONDA_DEFAULT_ENV", "unknown")
		with open(result_path / "conda_env_name.txt", "w") as f:
			f.write(env_name + "\n")

	except Exception:
		# Not in conda, or conda unavailable — nothing to record.
		pass
