...
# Optional: the qiskit hardware backend. Guarded so the main training paths
# import cleanly when qiskit is not installed.
try:
    from .qkan_fast_qiskit import qiskit_solver as _fast_qiskit_solver
    _FAST_QISKIT_AVAILABLE = True
except ImportError:
    _fast_qiskit_solver = None  # type: ignore[assignment]
    _FAST_QISKIT_AVAILABLE = False
...
class QKANLayer(nn.Module):
    ...
    def __init__(
        ...
        # Options for the optional qiskit hardware backend. Left empty here;
        # a caller that sets self.solver = "qiskit" fills this in. Keys:
        # backend, estimator, shots, optimization_level, parallel_qubits,
        # resilience_level, twirling, initial_layout.
        self._qiskit_kwargs: dict = {}

        # Options for the optional cudaq hardware backend. Left empty here;
        # a caller that sets self.solver = "cudaq" fills this in. Keys:
        # device_arn, shots, parallel_qubits, qubit_ranking, mitigation.
        self._cudaq_kwargs: dict = {}
    ...
    def forward(self, x: torch.Tensor, theta=None, base_weight=None):
        ...
        if self.solver == "qml":
            ...
            postacts = cutile_batched_solver(
                x,
                th,
                self.preacts_weight,
                self.preacts_bias,
            ...
            if not _FAST_QISKIT_AVAILABLE:
                raise ImportError(
                    "qiskit_solver not importable; install qiskit first."
                )
            postacts = _fast_qiskit_solver(
                x,
                th,
                self.preacts_weight,
                self.preacts_bias,
                ...
                out_dim=self.out_dim,
                dtype=self.c_dtype,
                solver_kwargs=self._qiskit_kwargs,
            ).to(self.p_dtype)
        elif self.solver == "cudaq":
            # User-facing solver name is "cudaq" (matches the target family
            # and the upstream qkan solver guide). The actual implementation
            # bypasses upstream cudaq_solver to get qubit pinning via direct
            # Amazon Braket SDK submission. The backend module is optional and
            # is not shipped in this repository.
            from .qkan_fast_braket import braket_fast_solver as _fast_cudaq_solver
            postacts = _fast_cudaq_solver(
                x,
                th,
                self.preacts_weight,
                self.preacts_bias,
                ...
                group=self.group,
                preacts_trainable=self.preact_trainable,
                fast_measure=False,          # FORCED OFF on HW (Born rule)
                out_dim=self.out_dim,
                dtype=self.c_dtype,
                solver_kwargs=getattr(self, "_cudaq_kwargs", {}),
            ).to(self.p_dtype)
