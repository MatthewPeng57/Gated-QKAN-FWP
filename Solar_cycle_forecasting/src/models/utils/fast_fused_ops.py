...
def _pz_encoding_kernel(
    # Pointers
    x_ptr,  # [Batch, In_Dim]
    theta_ptr,  # [Batch, Out_Dim, In_Dim, Reps+1, 2]
    ...
    theta_base = theta_ptr + b_offs * stride_t_b + idx_o * stride_t_o + idx_i * stride_t_i

    for layer in range(reps):
        # Rz(t0) — scalar theta trig, broadcast to batch tile
        # t0 = tl.load(theta_base + layer * stride_t_r + 0 * stride_t_p)
        t0 = tl.load(theta_base + layer * stride_t_r + 0 * stride_t_p, mask=b_mask, other=0.0)
        a = t0 * 0.5
        c = tl.cos(a)
        s = tl.sin(a)
        nr0 = r0 * c + i0 * s
        ...
        t1 = tl.load(theta_base + layer * stride_t_r + 1 * stride_t_p, mask=b_mask, other=0.0)
        a = t1 * 0.5
        c = tl.cos(a)
        s = tl.sin(a)
        nr0 = c * r0 - s * r1
    ...
    # Final Rz(t0), Ry(t1)
    t0 = tl.load(theta_base + reps * stride_t_r + 0 * stride_t_p, mask=b_mask, other=0.0)
    t1 = tl.load(theta_base + reps * stride_t_r + 1 * stride_t_p, mask=b_mask, other=0.0)
...
def _pz_encoding_backward_kernel(
    ...
    grad_x_local = tl.zeros([BLOCK_B], dtype=tl.float32)
    # gt_base = grad_theta_ptr + idx_o * stride_gt_o + idx_i * stride_gt_i
    gt_base = grad_theta_ptr + b_offs * stride_gt_b + idx_o * stride_gt_o + idx_i * stride_gt_i
